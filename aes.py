...
### Substitution Sbox function ###
def sub_bytes(s):
    for i in range(4):
        for j in range(4):
            s[i][j] = Sbox[s[i][j]]

### Inverse Sbox substitution function ### 
def inv_sub_bytes(s):
    for i in range(4):
        for j in range(4):
            s[i][j] = InvSbox[s[i][j]]
...
### matrix of bytes to array of bytes ###
def matrix2array(matrix):
    ...
    for i in range(4):
        for j in range(4):
            array = np.append([array],[matrix[i][j]])
...
### usless function for this code ###
def bytes_div(integer):
...
### mix columns computationally###
def mix_columns_comp(a):
    for i in range(4):
        mix_single_column_comp(a[i])

### mix columns using table_2 and table_3 from precalculated Rijndael values ###
def mix_single_column(a):
    a[0] = table_2[a[0][0]] + table_3[a[0][1]] + a[0][2] + a[0][3]
    a[1] = a[1][0] + table_2[a[1][1]] + table_3[a[1][2]] + a[1][3]  
    a[2] = a[2][0] + a[2][1] + table_2[a[2][2]] + table_3[a[2][3]]
    a[3] = table_3[a[3][0]] + a[3][1] + a[3][2] + table_2[a[3][3]]

### inverse mix columns using table 14 from precalculated Rijndael values ###

def inv_mix_single_column(a):
    a[0] = table_14[a[0][0]] + table_11[a[0][1]] + table_13[a[0][2]] + table_9[a[0][3]]
    a[1] = table_9[a[1][0]] + table_14[a[1][1]] + table_11[a[1][2]] + table_13[a[1][3]]  
    a[2] = table_13[a[2][0]] + table_9[a[2][1]] + table_14[a[2][2]] + table_11[a[2][3]]
    a[3] = table_11[a[3][0]] + table_13[a[3][1]] + table_9[a[3][2]] + table_14[a[3][3]]
    mix_columns(a)
...
def xorBytes(matrixA,matrixB):
    ...
    for i in range(4):
        for j in range(4):
            matrixtemp[i][j] = matrixA[i][j] ^ matrixB[i][j]
